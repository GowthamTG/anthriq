...
const descriptions = {
  idle: 'Your instrument is ready. Start a continuous recording with the configuration below.',
...
export default function Acquire() {
  ...
  useEffect(() => {
    ...
    events.addEventListener('state', event => { setState(JSON.parse(event.data)); setConnected(true); });
  ...
  async function command(action: 'start' | 'stop') {
    ...
    try {
      const response = await fetch(action === 'start' ? '/api/acquisitions' : `/api/acquisitions/${state?.id}/stop`, { method: 'POST' });
  ...
  return <>
    ...
    <main className="mx-auto max-w-[1456px] px-12 max-[1050px]:px-7 max-[760px]:px-5 max-[760px]:pb-[90px]">
      <div className="page-heading flex items-center justify-between pt-[49px] pb-[37px] max-[760px]:pt-8 max-[760px]:pb-[25px]">
        ...
        <div className="session-label grid gap-[9px] text-right max-[760px]:hidden"><span className="micro">ACQUISITION MODE</span><strong>Continuous</strong><span>Until you press Stop</span></div>
      </div>
      ...
      <div className="workspace grid grid-cols-[minmax(0,1.8fr)_minmax(320px,1fr)] border border-line max-[1050px]:grid-cols-[minmax(0,1.4fr)_minmax(300px,1fr)] max-[760px]:grid-cols-1">
        <section className="instrument min-w-0 bg-panel" aria-label="Acquisition measurements">
          ...
          <div className="primary-reading px-[30px] pt-[35px] pb-[30px] max-[1050px]:px-[22px] max-[760px]:px-5 max-[760px]:py-[25px]">
          ...
          <div className="measurements grid grid-cols-3 gap-5 px-[30px] pb-7 max-[1050px]:gap-2.5 max-[1050px]:px-[22px] max-[760px]:gap-2 max-[760px]:px-5 max-[760px]:pb-[25px]">
            ...
            <div><span className="micro">RECORDED FRAMES</span><output>{number(frames)}</output><span>32 values per frame</span></div>
            <div><span className="micro">FRAME DATA</span><output>{bytes(size)}</output><span>136 bytes per frame</span></div>
          </div>
          <div className="buffer border-t border-line px-[30px] py-[25px] max-[1050px]:px-[22px] max-[760px]:px-5 max-[760px]:py-[23px]">
            <div><span className="micro">RECORDING BUFFER</span><span>{queued == null ? 'Awaiting acquisition' : `${bytes(queued)} / 4.19 MB`}</span></div>
        ...
        <aside className="control-panel border-l border-line bg-[#191b19] px-7 py-[26px] max-[1050px]:px-[22px] max-[1050px]:py-[25px] max-[760px]:border-t max-[760px]:border-l-0 max-[760px]:px-5" aria-label="Acquisition controls">
          <div className="control-heading flex items-center justify-between"><span className="micro">INSTRUMENT SETUP</span><span className="default-label border border-[#424641] px-[7px] py-[5px] font-mono text-[9px] tracking-[1px] text-muted">DEFAULT</span></div>
          ...
          <dl className="configuration my-6 max-[760px]:mt-4">
            <div><dt>Channels</dt><dd>32 <small>CH</small></dd></div>
            <div><dt>Sample rate <small>PER CHANNEL</small></dt><dd>4,000 <small>Hz</small></dd></div>
            <div><dt>Aggregate rate</dt><dd>128,000 <small>VALUES/S</small></dd></div>
            <div><dt>Signal seed</dt><dd>42</dd></div>
          </dl>
          <div className="controls grid gap-[9px] max-[760px]:fixed max-[760px]:inset-x-0 max-[760px]:bottom-0 max-[760px]:z-10 max-[760px]:grid-cols-[1.2fr_1fr] max-[760px]:border-t max-[760px]:border-line max-[760px]:bg-background max-[760px]:px-5 max-[760px]:pt-3.5 max-[760px]:pb-[max(14px,env(safe-area-inset-bottom))] max-[760px]:[&_button]:min-h-[46px]">
            <button className="start-button" aria-label="Start acquisition" disabled={!connected || busy || active} onClick={() => command('start')}><span className="button-marker size-[7px] border border-current" aria-hidden="true" />{status === 'starting' ? 'Starting acquisition…' : 'Start acquisition'}<span aria-hidden="true">↗</span></button>
