...
const values = [
  -0.9200000166893005, -0.4000000059604645, 0.11999999731779099, 0.4000000059604645,
  0.6800000071525574, 0.4000000059604645, 0.11999999731779099, -0.4000000059604645,
];
...
async function fixture(t, records = values.map((value, index) => ({ index, value })), update = {}) {
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      id: 'fixture',
      displayName: 'Independent fixture',
      status: 'completed',
      startedAt: '2026-09-13T00:00:00.000Z',
      stoppedAt: '2026-09-13T00:00:01.000Z',
      channels: 1,
      sampleRate: 8,
      seed: 0,
      bufferBytes: 4096,
      ...
      stallForMs: 0,
      expectedFrames: 8,
      recordedFrames: records.length,
      totalSamples: records.length,
      droppedFrames: 8 - records.length,
      duration: 1,
      sampleType: 'float32',
      bytesPerSample: 4,
...
test('duplicates and incorrect values overlap while keeping their first physical positions', async (t) => {
  ...
  const directory = await fixture(t, records, {
    recordedFrames: 9,
    totalSamples: 9,
    droppedFrames: 0,
  });
  await assert.rejects(cli('verify', directory), (error) => {
    ...
    assert.deepEqual(report.discrepancies.incorrect.first, {
      frame: 1,
      channel: 0,
      physicalOrdinal: 2,
      expected: values[1],
...
test('format failures produce reports, while malformed record identity suppresses invented totals', async (t) => {
  ...
  const decreasing = await fixture(
    t,
    [0, 2, 1, 3].map((index) => ({ index, value: values[index] })),
    { recordedFrames: 4, totalSamples: 4, droppedFrames: 4 },
  );
...
test('partial records and unconfirmed extent fail explicitly without treating a readable prefix as verified', async (t) => {
  const directory = await fixture(
    t,
    values.slice(0, 4).map((value, index) => ({ index, value })),
    { expectedFrames: null, duration: null, recordedFrames: 4, totalSamples: 4, droppedFrames: 0 },
  );
...
test('unsafe and out-of-extent frame identities fail without precise discrepancy claims', async (t) => {
  ...
  const outside = await fixture(
    ...
    [
      ...
      { index: 8, value: values[0] },
    ],
    { recordedFrames: 2, totalSamples: 2, droppedFrames: 6 },
  );
...
test('inspection marks a saved result stale after frame state changes and a rerun replaces it', async (t) => {
  ...
  assert.equal(
    JSON.parse((await cli('inspect', directory)).stdout).verification.status,
    'verified',
  );
