...
fs.open = async function (path, ...args) {
  ...
  if (
    ['short', 'ENOSPC'].includes(process.env.SCOPE_TEST_DISK_FAULT) &&
    String(path).endsWith('/frames.bin') &&
    args[0] === 'wx'
  ) {
    ...
    handle.write = async (buffer, offset, length, ...rest) => {
      if (process.env.SCOPE_TEST_DISK_FAULT === 'ENOSPC' && bytes >= 141) {
        throw Object.assign(
          new Error('ENOSPC: no space left on device, write frames.bin (controlled test)'),
          { code: 'ENOSPC' },
        );
      }
      const result = await write(
        buffer,
        offset,
        Math.min(length, process.env.SCOPE_TEST_DISK_FAULT === 'ENOSPC' ? 141 - bytes : 17),
        ...rest,
      );
...
fs.rename = async function (from, to) {
  if (process.env.SCOPE_TEST_DISK_FAULT === 'finalize' && String(to).endsWith('/metadata.json')) {
    ...
    if (metadata.status === 'completed')
      throw new Error('EACCES: metadata finalization denied (controlled test)');
  }
  return originalRename.call(this, from, to);
};
syncBuiltinESMExports();
if (
  process.env.SCOPE_TEST_DISK_FAULT === 'recorder-ipc' &&
