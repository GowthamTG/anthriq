...
export async function createDiagnosticScenario(
  root: string,
  ...
  if (source.status !== 'completed')
    throw Object.assign(new Error('Only completed recordings can seed a diagnostic scenario'), {
      statusCode: 409,
    });
  if (source.diagnostic)
    throw Object.assign(new Error('Choose an acquisition recording as the diagnostic source'), {
      statusCode: 409,
    });
  ...
  try {
    ...
    const frames = Buffer.concat(
      definition.indices.map((index) => encodeFrames(index, 1, settings)),
    );
      ...
      definition.losses
        .map(([start, end]) =>
          JSON.stringify({
            ...
            frames: end - start,
            samples: (end - start) * source.channels,
            cause: 'disposable diagnostic scenario',
          }),
        )
