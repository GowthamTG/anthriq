...
export function parseMetadata(raw: unknown): RecordingMetadata {
  ...
  const integer = (key: string, min: number, max: number) => {
    const value = m[key];
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < min || value > max) invalid(`Invalid metadata ${key}`);
    return value as number;
  };
  const finite = (key: string) => {
    if (typeof m[key] !== 'number' || !Number.isFinite(m[key]) || m[key] < 0) invalid(`Invalid metadata ${key}`);
  };
  ...
  finite('seconds'); finite('writeDelayMs');
  if (m.sampleType !== 'float32' || m.bytesPerSample !== 4 || m.byteOrder !== 'little-endian' || m.recordBytes !== stride(channels) || m.layout !== 'uint64 frame index, then interleaved channel values') invalid('Unsupported or contradictory recording layout');
