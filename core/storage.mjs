...
import { stride } from './signal.mjs';

export async function writeAll(file, buffer) {
  let offset = 0;
...
export async function saveMetadata(directory, metadata) {
...
async function readExact(file, buffer, position, bytes = buffer.length) {
  let offset = 0;
  while (offset < bytes) {
...
export async function lowerBound(file, count, width, frame) {
...
export function selection(metadata, options = {}) {
  ...
  const end = options.end ?? metadata.expectedFrames;
...
export async function* readFrames(directory, options = {}) {
...
export async function* scanFrames(directory, metadata) {
  const file = await open(join(directory, 'frames.bin'), 'r');
