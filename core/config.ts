...
export function safeExtent(frames: number, channels: number) {
  return Number.isSafeInteger(frames) && frames >= 0 && Number.isSafeInteger(frames * channels) && Number.isSafeInteger(frames * (8 + 4 * channels));
}
...
export function config(input: unknown = {}): Settings {
  ...
  for (const [key, min, max] of [['channels', 1, 256], ['sampleRate', 1, 100000], ['seed', 0, 2147483647], ['bufferBytes', 4096, 67108864]] as const) {
    if (!Number.isSafeInteger(result[key]) || result[key] < min || result[key] > max) fields[key] = `${key} must be an integer from ${min} to ${max}`;
  }
  ...
  if (raw.displayName !== undefined && (typeof raw.displayName !== 'string' || raw.displayName.length > 120)) {
  ...
  return { ...result, displayName: typeof raw.displayName === 'string' ? raw.displayName.trim() : '' };
}
