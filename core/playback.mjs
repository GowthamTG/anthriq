...
export class Playback {
  ...
  async init() {
    this.metadata = await inspect(this.directory);
    this.channels = Array.from({ length: this.metadata.channels }, (_, i) => i);
    this.end = this.metadata.expectedFrames ?? this.metadata.recordedFrames;
    return this;
  }
  status() {
    return {
      position: this.position,
      seconds: this.position / this.metadata.sampleRate,
      duration: this.end / this.metadata.sampleRate,
      speed: this.speed,
      playing: this.playing,
      emittedFrames: this.emittedFrames,
      duplicates: this.duplicates,
      maxLagMs: this.maxLagMs,
      lagFrames: this.lagFrames || 0,
    };
  }
  anchor() {
  ...
  seek(frame) {
    if (!Number.isSafeInteger(frame) || frame < 0 || frame > this.end)
      throw new Error('Seek is outside recording');
    this.generation++;
    this.position = frame;
    this.anchor();
    this.onStatus(this.status());
  ...
  setSpeed(speed) {
    if (!Number.isFinite(speed) || speed < 0.1 || speed > 8)
      throw new Error('Playback speed must be between 0.1× and 8×');
    this.generation++;
    this.speed = speed;
    this.anchor();
    this.onStatus(this.status());
  ...
  setChannels(channels) {
    selection(this.metadata, { start: 0, end: 0, channels });
    this.generation++;
  ...
  async tick() {
    ...
    const target = Math.min(
      ...
      Math.floor(
        this.basePosition +
          ((performance.now() - this.baseTime) / 1000) * this.metadata.sampleRate * this.speed,
      ),
    );
    ...
    try {
      let frames = [],
        previous = -1;
      for await (const frame of readFrames(this.directory, {
        start,
        end,
        ...
        if (frames.length === 256) {
          ...
          this.emittedFrames += frames.length;
          frames = [];
        }
      }
      if (generation !== this.generation || !this.playing) return;
      ...
      this.position = end;
      const nowTarget = Math.min(
        this.end,
        this.basePosition +
          ((performance.now() - this.baseTime) / 1000) * this.metadata.sampleRate * this.speed,
      );
      this.lagFrames = Math.max(0, nowTarget - this.position);
      this.maxLagMs = Math.max(
        this.maxLagMs,
        (this.lagFrames / (this.metadata.sampleRate * this.speed)) * 1000,
      );
