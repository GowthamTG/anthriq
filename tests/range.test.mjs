...
test('retrieve uses half-open index/time ranges and preserves requested channel order', async (t) => {
  ...
  const index = (
    await cli('retrieve', directory, '--start', '2', '--end', '5', '--channels', '3,1')
  ).stdout
    .trim()
    .split('\n')
    .map(JSON.parse);
  assert.deepEqual(
    index.map((frame) => frame.index),
    [2, 3, 4],
  );
  assert.equal(index[0].values.length, 2);
  const time = (
    await cli('retrieve', directory, '--start-seconds', '0.021', '--end-seconds', '0.05')
  ).stdout
    .trim()
    .split('\n')
    .map(JSON.parse);
  assert.deepEqual(
    time.map((frame) => frame.index),
    [3, 4],
  );
...
test('retrieve rejects conflicting and invalid selections', async (t) => {
  const directory = await fixture(t);
  for (const args of [
    ['--start', '1', '--start-seconds', '0.1'],
    ['--channels', '1,1'],
    ['--channels', ''],
    ['--end', '-1'],
  ]) {
    await assert.rejects(cli('retrieve', directory, ...args), (error) => {
      assert.equal(error.code, 1);
      return true;
    });
  }
  for (const channels of ['1,', '1,,2', '01'])
    await assert.rejects(cli('retrieve', directory, '--channels', channels), { code: 1 });
});
...
test('retrieve preserves gaps and adjacent duplicate observations', async (t) => {
  ...
  await writeFile(
    join(directory, 'frames.bin'),
    Buffer.concat([
      frames.subarray(0, width),
      frames.subarray(2 * width, 3 * width),
      frames.subarray(2 * width, 3 * width),
      frames.subarray(5 * width, 6 * width),
    ]),
  );
  const result = (await cli('retrieve', directory, '--start', '0', '--end', '6')).stdout
    .trim()
    .split('\n')
    .map(JSON.parse);
  assert.deepEqual(
    result.map((frame) => frame.index),
    [0, 2, 2, 5],
  );
});
...
test('a current verifier ordering failure blocks seeking even when it is not the first format error', async (t) => {
  ...
  await assert.rejects(cli('retrieve', directory, '--start', '0', '--end', '5'), (error) => {
    assert.match(error.stderr, /malformed frame ordering/i);
    return true;
  });
});
...
test('CSV rows retain selected channel order and original timeline units', async (t) => {
  const directory = await fixture(t);
  const frames = (
    await cli('retrieve', directory, '--start', '2', '--end', '3', '--channels', '3,1')
  ).stdout
    .trim()
    .split('\n')
    .map(JSON.parse);
  ...
  const lines = [];
  for await (const line of csvLines(directory, { start: 2, end: 3, channels: [3, 1] }))
    lines.push(line);
