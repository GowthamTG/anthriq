...
async function capture(t, options = [], env = {}) {
  ...
  const child = spawn(process.execPath, ['core/cli.ts', 'record', directory, ...options], { env: { ...process.env, ...env }, stdio: ['ignore', 'pipe', 'pipe'] });
  const closed = once(child, 'close');
  let stderr = '';
  child.stderr.on('data', d => { stderr += d; });
  let pids = [];
  t.after(async () => {
    for (const pid of [...pids, child.pid]) { try { process.kill(pid, 'SIGKILL'); } catch {} }
    await closed;
    await rm(root, { recursive: true, force: true });
  });
  const metadata = await until(async () => JSON.parse(await readFile(join(directory, 'metadata.json'), 'utf8')), m => m.processes);
...
async function inspection(directory) {
  return JSON.parse((await execute(process.execPath, ['core/cli.ts', 'inspect', directory], { timeout: 5000 })).stdout);
}

test('recorder death leaves a failed inspectable prefix and terminates its generator', { timeout: 15000 }, async t => {
  ...
  await until(async () => { try { process.kill(run.metadata.processes.generator, 0); return false; } catch { return true; } }, Boolean);
});

test('generator death during a delayed write preserves a readable failed prefix', { timeout: 15000 }, async t => {
...
const faultEnvironment = mode => ({ NODE_OPTIONS: `--import=${new URL('./fixtures/disk-fault.mjs', import.meta.url).href}`, SCOPE_TEST_DISK_FAULT: mode });
test('disk exhaustion after a partial write preserves only complete frames and reports the cause', { timeout: 15000 }, async t => {
...
test('finalization failure is explicit and keeps the independently confirmed extent', { timeout: 15000 }, async t => {
...
test('recorder death also terminates a suspended generator without waiting for IPC cleanup', { timeout: 4000 }, async t => {
  const run = await capture(t);
...
for (const [signal, pause] of [['SIGINT', 0], ['SIGTERM', 100]]) {
  test(`${signal} before or during delayed writes drains accepted frames cleanly`, { timeout: 15000 }, async t => {
    const run = await capture(t, ['--channels', '1', '--sample-rate', '20', '--write-delay-ms', '300']);
    await sleep(pause);
    ...
    assert.equal(saved.trailingBytes, 0);
    for (const pid of Object.values(saved.processes)) assert.throws(() => process.kill(pid, 0), { code: 'ESRCH' });
  });
}

test('a timed drain exceeding the deadline fails but retains the confirmed source extent', { timeout: 15000 }, async t => {
  ...
  await assert.rejects(execute(process.execPath, ['core/cli.ts', 'verify', run.directory]), error => {
