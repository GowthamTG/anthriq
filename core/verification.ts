...
import { DIAGNOSTIC_SCENARIOS, type DiagnosticScenario, type VerificationState, type VerificationWorkerCommand, type VerificationWorkerMessage } from './contracts.ts';
...
export class Verification {
  ...
  state: VerificationState = { status: 'idle', recordingId: null, scenario: null, sourceRecordingId: null, workerPid: null, progress: null, report: null, error: null };
  ...
  active() { return Boolean(this.child) || this.state.status === 'creating' || this.state.status === 'running'; }

  async start(id: string) {
    if (!recordingId(id)) throw Object.assign(new Error('Recording not found'), { statusCode: 404 });
    if (this.active()) throw Object.assign(new Error('A verification is already active'), { statusCode: 409 });
    this.state = { status: 'running', recordingId: id, scenario: null, sourceRecordingId: null, workerPid: null, progress: null, report: null, error: null };
    this.publish();
    ...
    try {
      ...
      this.state = { status: 'idle', recordingId: null, scenario: null, sourceRecordingId: null, workerPid: null, progress: null, report: null, error: null };
      this.publish();
      throw error;
    }
  ...
  async startDiagnostic(sourceRecordingId: string, scenario: DiagnosticScenario) {
    ...
    if (this.active()) throw Object.assign(new Error('A verification is already active'), { statusCode: 409 });
    this.state = { status: 'creating', recordingId: null, scenario, sourceRecordingId, workerPid: null, progress: null, report: null, error: null };
    this.publish();
    try {
