...
export async function verifyRecording(
  directory: string,
  options: { onProgress?: (progress: VerificationProgress) => void } = {},
): Promise<VerificationReport> {
  const started = performance.now();
  const metadataPath = join(directory, 'metadata.json');
  const framesPath = join(directory, 'frames.bin');
  const [metadataFile, framesFile] = await Promise.all([
    open(metadataPath, 'r'),
    open(framesPath, 'r'),
  ]);
  ...
  try {
    ...
    try {
      metadata = await readMetadataHandle(metadataFile);
    } catch (error) {
    ...
    if (metadata) {
      if (metadata.id !== basename(directory))
        formatError('Metadata recording identity differs from the bundle directory');
      ...
      if (metadata.recordedFrames !== completeRecords)
        formatError('Metadata record count differs from physical complete-record count');
      if (metadata.totalSamples !== metadata.recordedFrames * metadata.channels)
        formatError('Metadata scalar count contradicts its frame count');
      if (
        metadata.expectedFrames !== null &&
        metadata.duration !== metadata.expectedFrames / metadata.sampleRate
      )
        formatError('Metadata duration contradicts the expected frame extent');
      ...
      if (
        ...
        formatError('Recorded and lost frame counts contradict the expected extent');
    }
      ...
      options.onProgress?.({
        ...
        percent:
          completeRecords === null
            ...
            : completeRecords === 0
              ? 100
              : (recordsScanned / completeRecords) * 100,
    ...
    if (metadata && completeRecords !== null && width !== null) {
      ...
      for (let ordinal = 0; ordinal < completeRecords; ordinal += recordsPerChunk) {
        ...
        while (offset < wanted) {
          const { bytesRead } = await framesFile.read(
            buffer,
            offset,
            wanted - offset,
            ordinal * width + offset,
          );
            ...
            for (let channel = 0; channel < metadata.channels; channel++) {
              ...
              if (!Object.is(actual, expected)) {
                ...
                firstIncorrect ??= {
                  ...
                  physicalOrdinal,
                  expected,
                  actual: Number.isFinite(actual) ? actual : String(actual),
                };
              }
            }
          }
      ...
      if (
        identityTrusted &&
        missing !== null &&
        (metadata.droppedFrames ?? 0) !== missing / metadata.channels
      ) {
    ...
    const [metadataAfter, framesAfter, metadataPathAfter, framesPathAfter] = await Promise.all([
      metadataFile.stat({ bigint: true }).then((value) => identity(value as never)),
      framesFile.stat({ bigint: true }).then((value) => identity(value as never)),
      pathIdentity(metadataPath),
      pathIdentity(framesPath),
    ]);
    if (
      !sameIdentity(metadataIdentity, metadataAfter) ||
      !sameIdentity(framesIdentity, framesAfter) ||
      !sameIdentity(metadataIdentity, metadataPathAfter) ||
      !sameIdentity(framesIdentity, framesPathAfter)
    ) {
    ...
    const report: VerificationReport = {
      ...
      counts: {
        ...
        expectedSamples:
          metadata?.expectedFrames == null ? null : metadata.expectedFrames * metadata.channels,
        recordedFrames: completeRecords,
        recordedSamples:
          completeRecords === null || !metadata ? null : completeRecords * metadata.channels,
      },
      discrepancies: {
      ...
      execution: {
        ...
        bytesScanned,
        totalBytes,
        percent: completeRecords === null ? null : 100,
        elapsedMs,
        rssBytes: finalRssBytes,
        peakRssBytes,
      },
      result:
        formatCount === 0 &&
    ...
    try {
      ...
      const [metadataBeforeSave, framesBeforeSave] = await Promise.all([
        pathIdentity(metadataPath),
        pathIdentity(framesPath),
      ]);
      if (
        !sameIdentity(metadataIdentity, metadataBeforeSave) ||
        !sameIdentity(framesIdentity, framesBeforeSave)
      )
        throw new Error('Recording changed before the verification report could be saved');
