...
test('temporary recorder stall drops exact intervals and recovers on the original clock', async (t) => {
  ...
  const losses = (await readFile(join(directory, 'losses.jsonl'), 'utf8'))
    .trim()
    .split('\n')
    .filter(Boolean)
    .map(JSON.parse);
  const indices = Array.from({ length: data.length / 136 }, (_, i) =>
    Number(data.readBigUInt64LE(i * 136)),
  );
  let next = 0;
  const gaps = [];
  ...
  assert.deepEqual(
    losses.map((x) => [x.startFrame, x.endFrameExclusive]),
    gaps,
  );
  ...
  assert.ok(
    indices.filter((i) => i >= 12000).length > 3500,
    'last second recovers useful throughput',
  );
  ...
  const telemetry = (await readFile(join(directory, 'metrics.jsonl'), 'utf8'))
    .trim()
    .split('\n')
    .map(JSON.parse);
  assert.ok(
    telemetry.some(
      (m) => m.recorderStall === 'active' && m.generator.emissionRateFramesPerSecond === 0,
    ),
  );
  assert.ok(
    telemetry.some(
      (m) => m.recorderStall === 'recovered' && m.generator.emissionRateFramesPerSecond > 0,
    ),
  );
