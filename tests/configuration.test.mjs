...
test('unknown acquisition options fail before creating a recording', async t => {
  ...
  await assert.rejects(cli('record', join(root, 'invalid'), '--seconds', '0.01', '--channelz', '8'), error => {
...
test('a missing CLI value cannot silently turn a timed capture into continuous capture', async t => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  await assert.rejects(cli('record', join(root, 'invalid'), '--seconds', '0.01', '--display-name'), error => {
...
test('invalid numeric inputs and unsafe timed extents are rejected before allocation', async t => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const invalid = [
    ['channels', '0'], ['channels', '257'], ['channels', '1.5'],
    ['sample-rate', '0'], ['sample-rate', '100001'], ['sample-rate', 'Infinity'],
    ['seed', '-1'], ['seed', '2147483648'], ['seed', 'NaN'],
    ['buffer-bytes', '4095'], ['buffer-bytes', '67108865'],
    ['seconds', '-1'], ['seconds', '1e300'], ['seed', ''],
  ];
  for (const [key, value] of invalid) {
    const args = key === 'seconds' ? [] : ['--seconds', '0.01'];
    await assert.rejects(cli('record', join(root, `invalid-${key}`), ...args, `--${key}`, value), { code: 1 });
  }
  assert.deepEqual(await readdir(root), []);
});
...
test('two nondefault timed acquisitions retain their settings, names, and independently checked values', async t => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const cases = [
    { channels: 8, rate: 1000, seed: 7, seconds: 0.5, frames: 500, values: [[0, 0, -0.8278760313987732], [499, 7, 0.6293341517448425]] },
    { channels: 3, rate: 200, seed: 123, seconds: 0.25, frames: 50, values: [[0, 2, 0.31989040970802307], [49, 2, -0.780164361000061]] },
  ];
  for (const fixture of cases) {
    const directory = join(root, `channels-${fixture.channels}`);
    const displayName = `Bench ${fixture.channels}`;
    await cli('record', directory, '--channels', String(fixture.channels), '--sample-rate', String(fixture.rate), '--seed', String(fixture.seed), '--seconds', String(fixture.seconds), '--display-name', displayName);
    ...
    for (const [frame, channel, expected] of fixture.values) {
      assert.equal(bytes.readFloatLE(frame * (8 + 4 * fixture.channels) + 8 + 4 * channel), expected);
    }
